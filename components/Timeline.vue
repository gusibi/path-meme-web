      ...
      <div v-for="post in blogPosts" :key="post.id" class="flex items-start">
        ...
        <div class="flex-grow bg-card-light dark:bg-card-dark rounded-lg px-6 py-8 shadow-xl">
          <h3 class="text-xl font-medium">
            <a :href="'/blog/' + post.number" class="hover:underline">{{ post.title }}</a>
          </h3>
          ...
          <div class="mt-4 flex justify-between items-center text-sm">
            ...
            <div class="flex items-center space-x-4">
              <a class="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300" :href="'/blog/' + post.number">💬 {{ post.comments || 0 }}</a>
              <div v-html="renderLabels(post.labels)"></div>
  ...
  methods: {
    ...
    renderLabels(labels) {
      return labels.map(label =>
        `<span class="card-label inline-block bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-full px-2 py-1 text-xs font-semibold mr-2" style="background-color: #${label.color}" title="${label.name}">
          <a href="/tag/${encodeURIComponent(label.name)}" class="text-gray-700 dark:text-gray-300 hover:underline">${label.name.length > 10 ? label.name.substring(0, 10) + '...' : label.name}</a>
        </span>`
      ).join('');
    },
    getLabelColor(labels) {
      return labels.length > 0 ? `#${labels[0].color}` : '#ccc';
    },
    getFirstLabelChar(labels) {
