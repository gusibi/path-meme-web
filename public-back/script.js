document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function loadBlogPosts(timeline) {
    try {
      const response = await fetch(
        "https://path-memo-api.gusibi.site/api/blog-posts",
      );
      if (!response.ok) {
    ...
    window.addEventListener("scroll", () => {
      ...
      if (visibleCard) {
        const dateTime =
          visibleCard.querySelector(".card-datetime").textContent;
  ...
  async function loadBlogPost(postId) {
    try {
      const response = await fetch(
        `https://path-memo-api.gusibi.site/api/blog-posts/${postId}`,
      );
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
  ...
  async function loadBlogPostsByTag(tagName, timeline) {
    try {
      const response = await fetch(
        `https://path-memo-api.gusibi.site/api/blog-posts?tag=${encodeURIComponent(tagName)}`,
      );
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
  ...
  function renderCardContent(post) {
    const titleHtml =
      post.title &&
      !post.labels.some((label) => label.name.toLowerCase() === "meme")
        ? `<h2 class="card-title"><a href="/blog/${post.number}" data-navigo>${post.title}</a></h2>`
        : "";

    const isMobile = window.innerWidth <= 768;
    const reactions = isMobile
      ? renderReactions({
          [Object.keys(post.reactions).find((key) => post.reactions[key] > 0)]:
            post.reactions[
              Object.keys(post.reactions).find((key) => post.reactions[key] > 0)
            ],
        })
    ...
    const labels = isMobile
      ? renderLabels(post.labels.slice(0, 1))
